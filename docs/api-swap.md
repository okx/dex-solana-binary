...
| `useTokenLedger` | Boolean | No | Default is `false`. When `true`, the swap instruction derives its input amount on-chain from the token-ledger delta (snapshot taken immediately before the swap). Mainly useful with [`POST /swap-instruction`](api-swap-instruction), where callers can insert deposit instructions after the snapshot. Ignored when `enableCyclicArbitrage=true` (cyclic mode manages the token ledger itself) |
