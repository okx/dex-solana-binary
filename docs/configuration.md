...
| `METRICS_ENABLED`        | `false`      | Enable Prometheus metrics recording and the `/metrics` endpoint. When disabled, all metric macros are no-ops and `/metrics` returns 503; `/health`, `/ready`, `/status` remain available                       |
